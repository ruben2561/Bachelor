...
def extract_features_images(images, labels):
...
#not working, need to add the labels somehow
def extract_features_for_trainig_data(path):
    images = loadImages.load_images(path, [])
    train_data = extract_features_images(images)
    np.save("train_data.npy", train_data)

def extract_and_return_features_for_trainig_data(images, labels):
    train_data = extract_features_images(images, labels)
    np.save("train_data.npy", train_data)
    return train_data